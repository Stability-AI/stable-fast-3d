...
namespace texture_baker_cpp
{
    ...
    tb_float2 triangle_centroid(const tb_float2 &v0, const tb_float2 &v1, const tb_float2 &v2)
    {
        return {(v0.x + v1.x + v2.x) * 0.3333f, (v0.y + v1.y + v2.y) * 0.3333f};
    }

    float BVH::find_best_split_plane(const BVHNode &node, int &best_axis, int &best_pos, AABB &centroidBounds)
    {
        ...
        for (int axis = 0; axis < 2; ++axis) // We use 2 as we have only x and y
        {
            ...
            #endif
            {
                ...
                for (int i = node.start; i < node.end; i++)
                {
                    ...
                    int binIdx = std::min(BINS - 1, (int)((triangle.centroid[axis] - boundsMin) * scale));
                    count[binIdx]++;
	...
	#endif
        {
            ...
            for (int i = node.start; i < node.end; i += 2)
            {
                ...
                if (i + 1 < node.end)
                {
                    ...
                    v0 = _mm_set_ps(leafTri1.v0.x, leafTri1.v0.y, leafTri2.v0.x, leafTri2.v0.y);
                    v1 = _mm_set_ps(leafTri1.v1.x, leafTri1.v1.y, leafTri2.v1.x, leafTri2.v1.y);
                    v2 = _mm_set_ps(leafTri1.v2.x, leafTri1.v2.y, leafTri2.v2.x, leafTri2.v2.y);
                    centroid = _mm_set_ps(leafTri1.centroid.x, leafTri1.centroid.y, leafTri2.centroid.x, leafTri2.centroid.y);
                }
                else
                {
                    // Otherwise do some duplicated work
                    v0 = _mm_set_ps(leafTri1.v0.x, leafTri1.v0.y, leafTri1.v0.x, leafTri1.v0.y);
                    v1 = _mm_set_ps(leafTri1.v1.x, leafTri1.v1.y, leafTri1.v1.x, leafTri1.v1.y);
                    v2 = _mm_set_ps(leafTri1.v2.x, leafTri1.v2.y, leafTri1.v2.x, leafTri1.v2.y);
                    centroid = _mm_set_ps(leafTri1.centroid.x, leafTri1.centroid.y, leafTri1.centroid.x, leafTri1.centroid.y);
                }
    ...
    void BVH::build(const tb_float2 *vertices, const tb_int3 *indices, const int64_t &num_indices)
    {
#ifdef TIMING
        ...
        for (size_t i = 0; i < num_indices; ++i)
        {
            ...
            triangles.push_back({vertices[idx.x], vertices[idx.y], vertices[idx.z], static_cast<int>(i), triangle_centroid(vertices[idx.x], vertices[idx.y], vertices[idx.z])});
        }
        ...
        while (!node_queue.empty())
        {
            ...
            // Stop condition: if the best cost is greater than or equal to the parent's cost
            ...
            float scale = BINS / (centroidBounds.max[best_axis] -
                                  centroidBounds.min[best_axis]);
            int i = node.start;
            int j = node.end - 1;

            // Sort the triangle_indices in the range [start, end) based on the best axis
            while (i <= j)
            {
                // use the exact calculation we used for binning to prevent rare inaccuracies
                int tri_idx = triangle_indices[i];
                tb_float2 tcentr = triangles[tri_idx].centroid;
                int binIdx = std::min(BINS - 1, (int)((tcentr[best_axis] - centroidBounds.min[best_axis]) * scale));
                if (binIdx < best_pos)
                    i++;
                else
    ...
    bool barycentric_coordinates(tb_float2 xy, tb_float2 v1, tb_float2 v2, tb_float2 v3, float &u, float &v, float &w)
    {
    ...
    bool BVH::intersect(const tb_float2 &point, float &u, float &v, float &w, int &index) const
    {
    ...
    torch::Tensor rasterize_cpu(torch::Tensor uv,
                                ...
                                int64_t bake_resolution)
    {
        ...
        torch::Tensor rast_result = torch::empty({bake_resolution, bake_resolution, 4}, torch::TensorOptions().dtype(torch::kFloat32).device(torch::kCPU));
...
#pragma omp parallel for
        ...
        torch::Tensor rast)
    {
        ...
        torch::Tensor pos_bake = torch::empty({height, width, 3}, torch::TensorOptions().dtype(torch::kFloat32).device(torch::kCPU));
